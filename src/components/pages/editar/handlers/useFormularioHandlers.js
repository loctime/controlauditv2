...
export const useFormularioHandlers = ({
  ...
  }, [puedeEditar, formularioSeleccionado?.id, setFormularioSeleccionado, user, userProfile]);
  ...
  const handleGuardarCambiosSeccion = useCallback(async (
    ...
    try {
      ...
      if (setModalEditarSeccionAbierto) setModalEditarSeccionAbierto(false);
  ...
  }, [puedeEditar, seccionesNormalizadas, formularioSeleccionado?.id, setFormularioSeleccionado, user, userProfile]);
  ...
  const handleGuardarCambiosPregunta = useCallback(async (
    ...
    try {
      ...
      setFormularioSeleccionado(prev => ({ ...prev, secciones: seccionesActualizadas }));
      if (setModalEditarPreguntaAbierto) setModalEditarPreguntaAbierto(false);
    ...
    try {
      ...
      const seccionesActualizadas = seccionesNormalizadas.map(seccion => {
        if (seccion.nombre === seccionSeleccionada.nombre) {
          return { ...seccion, preguntas: [...seccion.preguntas, nuevaPregunta] };
        }
        return seccion;
      });
      ...
      setFormularioSeleccionado(prev => ({ ...prev, secciones: seccionesActualizadas }));
      if (setModalAgregarPreguntaAbierto) setModalAgregarPreguntaAbierto(false);
      if (setNuevaPregunta) setNuevaPregunta('');
  ...
  const handleEliminarFormulario = useCallback(async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await formularioService.deleteFormulario(id, user, userProfile);
  ...
  }, [puedeEliminar, setFormularioSeleccionado, user, userProfile]);
  ...
  }, [puedeEliminar, seccionesNormalizadas, formularioSeleccionado?.id, setFormularioSeleccionado, user, userProfile]);

  const handleEliminarPregunta = useCallback(async (indexPregunta, nombreSeccion) => {
    logger.debug('🔧 [DEBUG] handleEliminarPregunta llamado, index:', indexPregunta, 'seccion:', nombreSeccion);
      ...
      try {
        const seccionesActualizadas = seccionesNormalizadas.map(seccion => {
          if (seccion.nombre === nombreSeccion) {
            return { ...seccion, preguntas: seccion.preguntas.filter((_, idx) => idx !== indexPregunta) };
          }
          return seccion;
        });
  ...
  const handleAgregarSeccion = useCallback(async (nombreSeccion) => {
    logger.debug('🔧 [DEBUG] handleAgregarSeccion llamado, seccion:', nombreSeccion);
    ...
    try {
      ...
      const seccionesActualizadas = [...seccionesNormalizadas, { nombre: nombreSeccion.trim(), preguntas: [] }];
