...
const initDatabaseWithRetry = async (retries = 3) => {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      logger.debug('✅ Base de datos offline inicializada correctamente');
      return;
    } catch (error) {
      logger.error(`❌ Error al inicializar base de datos offline (intento ${i + 1}/${retries}):`, error);
      if (i === retries - 1) {
        logger.error('❌ Falló la inicialización de la base de datos offline después de todos los intentos');
